...
export default function RollItem({ id, name, imageURL, price, sizes, rollCart }) {
	const availableSizes = React.useMemo(() => ['105гр.', '200гр.', '270гр.'], []);
	...
	const incrementQuantity = () => {
		if (activeSize !== null) {
			...
			setError(true); // Вызываем ошибку, если размер не выбраны
		}
	};
	...
	const handleAddToCart = async () => {
		if (activeSize !== null) {
			try {
				...
				// alert(`Товар "${name}" добавлена в корзину`);
				setSnackbarMessage(`Товар "${name}" добавлена в корзину`); // Устанавливаем сообщение для Snackbar
				setSnackbarOpen(true);
				setIsInCart(true);
			} catch (error) {
				console.error("Ошибка при добавлении товар в корзину:", error);
				// setSnackbarMessage(`Ошибка при добавлении товара "${name}" в корзину. Пожалуйсста, авторизуйтесь.`); // Устанавливаем сообщение об ошибке для Snackbar
				setLoggedError(true);
			}
		} else {
			setError(true);
	...
	React.useEffect(() => {
		setIsInCart(rollCart.some((item) => item.product_name === name && item.product_size === availableSizes[activeSize]));
	}, [activeSize, availableSizes, isInCart, name, rollCart]);
	...
	return (
		<div className="roll-block">
			...
			<div className="roll-block__bottom">
				{!isInCart ? (
					<>
						<div className="roll-block__price">{activeSize !== null ? totalPrice * quantity : `От ${price}`}₽</div>
